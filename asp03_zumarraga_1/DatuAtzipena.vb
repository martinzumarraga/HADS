...
Public Class DatuAtzipena
    ...
    Public Shared Sub Konektatu()
        Dim strconSGTA_DB_Erabiltzaileak As String = "Data Source=158.227.106.20; Initial Catalog=SGTA16_DB;Integrated Security=False;User ID=SGTA16;Password=Zadorra;Connect Timeout=15;Encrypt=False;TrustServerCertificate=False"
        Try
    ...
    Public Shared Function IkasleaMatrikulatutakoIrakasgaienEgokitzaileaEskuratu(ByVal strEmail As String) As SqlDataAdapter
        Dim strSQL As String
        Dim data As New SqlDataAdapter

        Try
            strSQL = "SELECT DISTINCT irakasgaiKodea FROM KlasekoTaldeak WHERE kodea IN (SELECT taldeKodea FROM IkasleakTaldeak WHERE email='" & strEmail & "')"
            data.SelectCommand = New SqlCommand(strSQL, connection)
            Return data
        Catch ex As Exception
            Throw New ErroreaIkaslearenIrakasgaiakLortzean()
        End Try
    End Function


    Public Shared Function UstiapenekoLanGenerikoaenEgokitzaileaEskuratu() As SqlDataAdapter
        Dim strSQL As String
        Dim data As New SqlDataAdapter
        Try
            strSQL = "SELECT * FROM LanGenerikoak WHERE ustiapenean='True'"
            data.SelectCommand = New SqlCommand(strSQL, connection)
            Return data
        Catch ex As Exception
            Throw New ErroreaIkaslearenIrakasgaiakLortzean()
